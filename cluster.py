...
def getjobid(cmd,args):
    ...
    if len(args) < 1:
        if len(jobids) > 1:
            print("%s: Jobid required"%(cmd,), file=sys.stderr)
            print("Cluster %s job IDs:"%(cluster.get('name'),))
            for jid in jobids:
                print("  "+jid)
            sys.exit(1)
        elif len(jobids) == 0:
            print("%s: No jobs on cluster %s"%(cmd,cluster.get('name')), file=sys.stderr)
        ...
        if jobid not in jobids:
            print("Bad jobid",jobid, file=sys.stderr)
            print(usage, file=sys.stderr)
            sys.exit(1)
    return jobid
...
if cmd == "install":
    ...
    if dirname not in dirnames:
        print("%s: Bad data tag %s directory %s"%(cmd,dataname,dirname), file=sys.stderr)
        print("Cluster %s data tag %s directories:"%(cluster.get('name'),dataname))
        for dn in dirnames:
            print("  "+dn)
        sys.exit(1)
    ...
    try:
        ...
        print("%s: Local destination directory required."%(cmd,), file=sys.stderr)
        print(usage, file=sys.stderr)
        sys.exit(1)
    if not os.path.exists(destdir):
        print("%s: Local destination directory %s not found."%(cmd,destdir), file=sys.stderr)
        print(usage, file=sys.stderr)
        sys.exit(1)
    datanames = cluster.datanames()
    ...
    if len(args) > 0 and args[0] in datanames:
        ...
        if len(args) < 2:
            if args[0] in jobnames:
                ...
                print("%s: Data directory for data tag %s required"%(cmd,dataname), file=sys.stderr)
                print(usage, file=sys.stderr)
                sys.exit(1)
        else:
            dirnames = cluster.dirnames(dataname)
            dirname = args[1]
            if dirname not in dirnames:
                print("%s: Data directory %s for data tag %s not found"%(cmd,dirname,dataname), file=sys.stderr)
                print(usage, file=sys.stderr)
                sys.exit(1)
    ...
    if basename in cluster.list_jobids():
        print("Job id %s already in use."%(basename,), file=sys.stderr)
        sys.exit(1)
