...
class AWSResources(object):
	...
	self.s3conn = S3Connection(access_key,secret_key)
	...
	return sum(map(int,map(self.instance_exists,self.workers)))

    def get_winpulsar(self):
	return self.winpulsar

    def winpulsar_exists(self):
	return sum(map(int,map(self.instance_exists,self.winpulsar)))
	...
	if self.cmi:
		...
		du = st['disk_usage']
		self.volume_usage = map(du.get,('used_percent','used','total'))
	    except:
		pass
	...
	return sum(map(int,map(self.astack_exists,self.cfstacks)))
	...
	if self.master_exists() or self.worker_exists() or self.winpulsar_exists():
		...
		if n == 1:
	            ...
	            s += "%d*workers"%(n,)
		if self.cmi:
		    s += " [CPU:%s]"%(",".join(map(lambda w: "%.0f%%"%(self._load.get(w,0),),self.workers)),)
		else:
		    s += " [CPU:%s]"%(",".join(map(lambda w: "%.0f%%"%(self._cpu.get(w,0),),self.workers)),)
	        needcomma = True
		...
		if n == 1:
		    ...
		    s += "%d*winpulsar"%(n,)
		s += " [CPU:%s]"%(",".join(map(lambda w: "%.0f%%"%(self._cpu.get(w,0),),self.winpulsar)),)
	    s += '\n'
